...
{
  public:
    explicit tcp_server(std::string ip_address, unsigned short port);
    ~tcp_server();
...
inline tcp_server::tcp_server(std::string ip_address, const unsigned short port)
    : _ip_address(std::move(ip_address)), _port(port)
{
    try
    {
    ...
    while (_message_queue->try_dequeue(mp))
    {
        ...
        if (topic_string == "sub")
        {
            ...
            for (auto &[connection, topics] : _connections)
            {
                ...
                {
                    spdlog::debug("[tcp_server] client endpoint: {} subscribed to topic: {}", connection->my_endpoint,
                        subsciption_message->get_topic_to_subscribe_to_string());
        ...
        {
            if (connection->my_endpoint != mp.endpoint_ ||
                connection->service_name == mp.header_.get_service_name_string())
            {
